...
struct Matrix sum_for_matrix_exp(struct Matrix A, unsigned int level)
{
    ...
    struct Matrix SUM = matrix_allocation(A.rows, A.cols);
    if (SUM.data == NULL) return MATRIX_NULL;

    memcpy(SUM.data, A.data, SUM.cols * SUM.rows * sizeof(MatrixData));

    for (unsigned int counter = 2; counter <= level; counter++) {
        C = matrix_mult(SUM, A);
        memcpy(SUM.data, C.data, SUM.cols * SUM.rows * sizeof(MatrixData));
        matrix_free(&C);
    }
...
{
    struct Matrix E = matrix_allocation(A.cols, A.rows);
