...
// A = B + C
struct Matrix matrix_sum(struct Matrix A, const struct Matrix B, const struct Matrix C)
{
...
// A = B - C
struct Matrix matrix_substraction(struct Matrix A, const struct Matrix B, const struct Matrix C)
{
    if (B.cols != C.cols || B.rows != C.rows) {
        matrix_error();
        return MATRIX_NULL;
...
// C = A * B
struct Matrix matrix_mult(struct Matrix C, const struct Matrix A, const struct Matrix B) {
    if (A.cols != B.rows) {
        matrix_error();
        return MATRIX_NULL;
    }
    ...
    for (size_t row = 0; row < A.rows; row++) {
        for (size_t col = 0; col < B.cols; col++) {
            for (size_t idx = 0; idx < A.cols; idx++) {
                C.data[row * C.cols + col] += A.data[row * A.cols + idx] * B.data[col + idx * B.cols]; 
            }   
        }
    }
    return C;
...
void determinant_print(const struct Matrix A)
{
...
struct Matrix matrix_exponent(struct Matrix A)
{
    ...
    if (term_prev.data == NULL) {
        matrix_delete(&exp);
        return MATRIX_NULL;
    };
    matrix_set_one(term_prev);

    struct Matrix term_next = matrix_create(A.rows, A.cols);
    ...
    for (int idx = 1; idx < 100; ++idx) {
        ...
        matrix_mult(term_next, term_prev, A);
        matrix_print(term_prev, "term_prev");
        matrix_print(term_next, "term_next");
        matrix_div_k(term_next, idx);
        matrix_print(term_next, "term_next/k!");
        memcpy(term_prev.data, term_next.data, sizeof(MatrixItem) * term_prev.cols * term_prev.rows);
        matrix_add(exp, term_prev);
        matrix_print(exp, "exp");
