...
void dijkstra(int start, int end, const vector<vector<int>>& adjacency_matrix) {
    ...
    priority_queue<Neighbor, vector<Neighbor>, CompareDistance> min_heap;
    ...
    {
        auto [current_vertex, current_distance] = min_heap.top();
        min_heap.pop();
...
void start() {
    ...
    while (true) {
        ...
        if (start >= 0 && start < matrix_size && end >= 0 && end < matrix_size) {
            dijkstra(start, end, adjacency_matrix);} 
        else{
