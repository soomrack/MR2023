...
#define SERVER_IP "192.168.221.232"     // IP адрес Raspberry
...
int main()
{
    ...
    SDL_Window* window = SDL_CreateWindow("Robot Controller", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 400, 400, SDL_WINDOW_SHOWN);
    if (!window) {
        ...
        while (SDL_PollEvent(&e) != 0) {  // Обрабатываем события SDL
            ...
            if (e.type == SDL_QUIT) {  // Закрытие окна
                ...
                switch (e.key.keysym.sym) {
                    ...
                    case SDLK_2:        command = 'g'; 
                                        do_not_stop = true; break;    // Автономная работа
