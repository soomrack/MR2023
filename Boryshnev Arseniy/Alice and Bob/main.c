...
{
    alice.bank_account +=
        (Money)(alice.bank_account * alice.bank_account_pp / 100.0 / 12.0);
};

Money alice_salary(const int year, const int month)
{
    if (month == 12)
        alice.salary += (Money)(alice.salary * inflation_pp / 100);
    if (month != 3)
...
void alice_flat_bills()
{
    ...
    alice.flat_expenditure +=
        (Money)(alice.flat_expenditure * inflation_pp / 100 / 12);
};
...
{
    if (month == 8)
        bob.bank_account -= (Money)(bob.flat * tax_pp / 100);
};
...
{
    if (month == 12)
        bob.flat += (Money)(bob.flat * (inflation_pp / 100));
};
