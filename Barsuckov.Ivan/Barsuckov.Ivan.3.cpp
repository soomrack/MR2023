...
public:
    ...
    void set_zero();
    Matrix tr();
    Matrix exp(const Matrix A, const unsigned int degree = 10);
    double det();
...
private:
    std::string msg;
public:
    int Matrix_Exception(std::string msg) : msg{ msg } {};
...
Matrix Matrix::exp(const Matrix A, const unsigned int degree = 10)
{
...
{
    size_t buff_one = diag; // запоминаетс€ номер строки
    ...
    if (data[diag * cols + diag] == 0.0) {
        ...
        double buff_two = 0; // запоминаетс€ значение в €чейке
