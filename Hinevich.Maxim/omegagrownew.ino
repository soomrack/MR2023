...
void setup() {
  ...
  systemState.lastPumpActivation = 0;
...
void morningWatering() {
  if (sensorValues.hours == 7 && (millis() - systemState.lastPumpActivation) > WATERING_TIME) {
    if (systemState.pumpOn && (millis() - systemState.lastPumpActivation) >= WATERING_TIME) {
      digitalWrite(WATER_PUMP_PIN, OFF_STATE); 
    } else if (!systemState.pumpOn) {
      digitalWrite(WATER_PUMP_PIN, ON_STATE); 
      systemState.lastPumpActivation = millis(); 
    }
  }
}