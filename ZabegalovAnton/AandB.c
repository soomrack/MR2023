...
void profit_deposit(struct Person* human, double percent_1, double percent_2)
{
    ...
    if(human->deposit >= 5*1000*1000*100) //увеличение ставки по вкладу за отстаток 5 млн
        percent = percent_2;
    else 
        percent = percent_1;
...
void spend_dentist(struct Person* human, int *year, int *starting_point)
{
