...
Matrix& Matrix::operator+=(const Matrix &x) {
    if (rows != x.rows || cols != x.cols)throw SIZE_UNMATCH;
...
Matrix Matrix::minor(const unsigned int cur_row, const unsigned int cur_col){
    ...
    for (unsigned int idx = 0; idx < rows * cols; idx++){
        if ((idx % cols == cur_col) || (idx / cols == cur_row)) continue;
        rez.data[k++] = data[idx];
    }
...
bool operator==(const Matrix &x, const Matrix &y){
    bool flag = ((x.rows == y.rows) && (x.cols == y.cols));
    if (!flag) return flag;
...
void test_exp(){
    ...
    bool final = ((A == standard) && (B == standard));
    if (final){
