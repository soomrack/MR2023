...
struct Sensors {
  int start_hours;
  unsigned long sec_from_start;
  int hours;
  int minutes;

  int time_of_water_hour;
  int time_of_water_minute;
  unsigned long time_for_delay_temp;
  unsigned long time_for_delay_pump;
...
void time_update() {
  sensors.sec_from_start = millis() / 1000;
  sensors.minutes = (int)(sensors.sec_from_start / 60 % 60);
  sensors.hours = (int)((sensors.start_hours + sensors.sec_from_start / 3600) % 24);
}
...
void control_light() 
{
  if (sensors.light < climate.border_light && sensors.hours > 12 && sensors.hours < 24)
    state.light |= ON;
}
