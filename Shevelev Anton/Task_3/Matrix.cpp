...
Matrix& Matrix::operator+=(const Matrix& A) // B += A
{
...
Matrix& Matrix::operator+(const Matrix& A) // SUM = B + A
{
    if (A.cols != cols || A.rows != rows)
        throw Matrix_Exception("Operator+: Incorrect sizes");
...
Matrix& Matrix::operator-(const Matrix& A)  // DIFF = B - A
{
    if (A.cols != cols || A.rows != rows)
        throw Matrix_Exception("Operator-: Incorrect sizes");
...
Matrix& Matrix::operator*(const Matrix& A) //C = B * A
{
    if (A.cols != rows)
...
Matrix& Matrix::operator/=(const MatrixItem k)  // A /= k
{
...
Matrix& Matrix::operator/(const MatrixItem k) // C = A / k
{
...
MatrixItem Matrix::det(Matrix& A) // DET (A)
{
    if (A.rows != A.cols)
...
Matrix& Matrix::exp(idx = 100) // exp = exp(A)
{
