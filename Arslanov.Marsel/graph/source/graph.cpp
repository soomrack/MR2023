...
Graph::Graph(const std::vector<Flight>& flights) {
    for (const Flight& flight : flights) {
        graph[flight.origin_city].emplace_back(flight.dest_city,
                                               flight.air_time);
        graph[flight.dest_city].emplace_back(flight.origin_city,
                                             flight.air_time);
    }
}
...
void Graph::dijkstra(const std::string& origin, const std::string& dest) const {
    ...
    for (std::pair<std::string, std::vector<std::pair<std::string, double>>>
             node : graph)
    ...
    while (!pq.empty()) {
        ...
        for (const std::pair<std::string, double>& air_time :
             graph.at(current.second)) {
    ...
    std::cout << "\b\b\b\b" << std::endl
              << "Total time: " << air_times[dest] << std::endl;
}
