...
#define MAVLINK_MSG_ID_HEARTBEAT_MIN_LEN 1
...
{
    return mavlink_msg_heartbeat_pack_status(system_id, component_id, _status, msg,  heartbeat->system_status);
}
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_heartbeat_send(chan, heartbeat->system_status);
#else
