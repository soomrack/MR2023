...
public:
    ...
    Flight(double AIR_TIME, double AIRLINE_ID, string UNIQUE_CARRIER_NAME, 
           double ORIGIN_CITY_MARKET_ID, string ORIGIN_CITY, 
           double DEST_CITY_MARKET_ID, string DEST_CITY);
    void display();
};
...
void Write_to_file(ofstream& logfile, vector<Flight>& Flights)
{
    ...
    for (auto flight : Flights) {
        logfile << flight.city_from_id << "\t"    << flight.city_to_id << "\t"      
                << flight.time << "\t"            << flight.carrier_id << "\t"      
                << flight.carrier_name << "\t"    << flight.city_from << "\t"
                << flight.city_to << endl;
    }
}
